...
fn calculate_default_inv_freq(cfg: &Config) -> Vec<f32> {
    ...
    (0..head_dim).step_by(2).map(|i| 1f32 / cfg.rope_theta.powf(i as f32 / head_dim as f32)).collect()
}
...
impl Cache {
    pub fn new(use_kv_cache: bool, dtype: DType, config: &Config, device: &Device) -> Result<Self> {
        ...
        let theta = match &config.rope_scaling {
            ...
            Some(rope_scaling) => {
                let low_freq_wavelen = rope_scaling.original_max_position_embeddings as f32 / rope_scaling.low_freq_factor;
                let high_freq_wavelen = rope_scaling.original_max_position_embeddings as f32 / rope_scaling.high_freq_factor;
                ...
                calculate_default_inv_freq(config)
                    ...
                    .map(|freq| {
                        ...
                        if wavelen < high_freq_wavelen {
                            ...
                            let smooth =
                                (rope_scaling.original_max_position_embeddings as f32 / wavelen - rope_scaling.low_freq_factor) / (rope_scaling.high_freq_factor - rope_scaling.low_freq_factor);
...
impl CausalSelfAttention {
    ...
    fn forward(&self, x: &Tensor, index_pos: usize, block_idx: usize, cache: &mut Cache) -> Result<Tensor> {
        ...
        let mut v = v.reshape((b_sz, seq_len, self.num_key_value_heads, self.head_dim))?.transpose(1, 2)?;
        ...
        if cache.use_kv_cache {
            if let Some((cache_k, cache_v)) = &cache.kvs[block_idx] {
                ...
                if v_seq_len > 2 * self.max_position_embeddings {
                    v = v.narrow(D::Minus1, v_seq_len - self.max_position_embeddings, self.max_position_embeddings)?.contiguous()?
                }
            }
...
impl Mlp {
    ...
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        Ok(Self { c_fc1, c_fc2, c_proj, span })
    }
}
...
impl Block {
    ...
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        Ok(Self { rms_1, attn, rms_2, mlp, span })
    }
}
